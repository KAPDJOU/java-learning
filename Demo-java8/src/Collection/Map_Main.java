...
public class Map_Main {
	...
	map.put("p0", lp.get(0));
	map.put("p1", lp.get(1));
	// map.forEach( (key, value) -> System.out.println(key + " -> " + value.getFirstname()) ) ; // > p0 -> Carson p1
	// -> Patty
	...
	// map.forEach( (key, value) -> System.out.println(key + " -> " + value.getFirstname()) ) ; // > p1 -> Moe p0 ->
	// Moe
	...
	// Map : computeIfPresent Si la clé est présente : associe une valeur calculée par exécution de la fonction.
	// Retourne la valeur. // the existing map Map<String, Map<String, Person>> map = ... ; // key, newValue
	// map.computeIfPresent( "un", map, (key, value) -> ... // la nouvelle valeur ) .put(".un", john) ;
	//
